  ...
  return (
    <div className="w-full h-dvh flex flex-col p-6 ml-4 rounded-xl overflow-hidden">
      <div className="flex items-center gap-3 pb-3 mb-1 border-b border-gray-700 shrink-0">
        ...
        <select
          className="rounded-lg border border-gray-400 bg-gray-200 px-3 py-2 text-sm text-gray-100 focus:border-blue focus:outline-none"
          value={statusFilter}
          onChange={e => setStatusFilter(e.target.value)}
        >
          <option className="bg-gray-200 text-gray-100" value="">All Status</option>
          <option className="bg-gray-200 text-gray-100" value="ACTIVE">Active</option>
          <option className="bg-gray-200 text-gray-100" value="INACTIVE">Inactive</option>
        </select>
      </div>
        ...
        {isLoading ? (
          <>
            {Array.from({ length: 4 }).map((_, i) => (
              <div
                key={`skeleton-${i}`}
                className="max-w-sm p-6 bg-gray-600 border border-gray-500 rounded-lg animate-pulse"
              >
                <div className="h-6 w-20 bg-gray-500 rounded mb-3" />
                <div className="h-7 w-40 bg-gray-500 rounded mb-2" />
                <div className="h-4 w-32 bg-gray-500 rounded mb-4" />
                <div className="h-9 w-24 bg-gray-500 rounded" />
              </div>
            ))}
          </>
        ) : (
          <>
            {filteredData.map(staff => (
              <div
                key={staff.id}
                className={`max-w-sm max-h-fit p-6 bg-gray-600 border border-gray-500 rounded-lg transition-all duration-200 hover:border-blue/50 hover:shadow-lg hover:shadow-blue/10 ${
              ...
              >
                ...
                <Button className="bg-gray-700" routeId={staff.id}>
                  Manage
            ...
            {filteredData.length === 0 && (
              <div className="col-span-2 text-center text-gray-100/60 text-lg mt-10">
                No staff found.
              </div>
            )}
          </>
        ...
        {totalPages > 1 && (
          <div className="col-span-2 flex items-center justify-center gap-4 py-4">
            <button
              ...
              className="rounded-lg border border-gray-500 bg-gray-600 px-4 py-2 text-sm text-gray-100 hover:border-blue hover:text-blue disabled:opacity-40 disabled:cursor-not-allowed transition-colors duration-150"
            >
              <Icons.ChevronLeft className="inline h-4 w-4" /> Previous
            </button>
            <span className="text-sm font-mono text-gray-100/60">
